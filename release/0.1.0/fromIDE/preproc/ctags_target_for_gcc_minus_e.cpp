# 1 "/Users/tom/Documents/GitHub/Phone-sized-MidiControl/Firmware/Arduino for MTM 8mu Plus/uMIDI/uMIDI.ino"
# 1 "/Users/tom/Documents/GitHub/Phone-sized-MidiControl/Firmware/Arduino for MTM 8mu Plus/uMIDI/uMIDI.ino"
/*
...
# 19 "/Users/tom/Documents/GitHub/Phone-sized-MidiControl/Firmware/Arduino for MTM 8mu Plus/uMIDI/uMIDI.ino" 2
# 20 "/Users/tom/Documents/GitHub/Phone-sized-MidiControl/Firmware/Arduino for MTM 8mu Plus/uMIDI/uMIDI.ino" 2
# 21 "/Users/tom/Documents/GitHub/Phone-sized-MidiControl/Firmware/Arduino for MTM 8mu Plus/uMIDI/uMIDI.ino" 2
# 22 "/Users/tom/Documents/GitHub/Phone-sized-MidiControl/Firmware/Arduino for MTM 8mu Plus/uMIDI/uMIDI.ino" 2
# 23 "/Users/tom/Documents/GitHub/Phone-sized-MidiControl/Firmware/Arduino for MTM 8mu Plus/uMIDI/uMIDI.ino" 2
...
Bounce * buttons = new Bounce[6 /* all buttons */];
...
bool midiBlink[8]; // should this led be blinking
...
void setup() {
  ...
  for (int i = 0; i < 6 /* all buttons */; i++) {
    buttons[i].attach( buttonPins[i] , (0x2) );
...
void loop() {
  ...
  // Read Faders
  for (int j = 0; j < 8; j++) {
    int thisFader = (((7 * flip) - j)>0?((7 * flip) - j):-((7 * flip) - j));
    analog[j]->update(analogRead(faders[thisFader]));
    int temp = analog[j]->getValue();
    temp = ((temp)<(faderMin)?(faderMin):((temp)>(faderMax)?(faderMax):(temp)));
...
# 1 "/Users/tom/Documents/GitHub/Phone-sized-MidiControl/Firmware/Arduino for MTM 8mu Plus/uMIDI/configuration.ino"
/*
...
void checkDefaultSettings() {
  ...
  if (firstByte > 0x01) {
    Serial.println("First Byte is > 0x01, probably needs initialising");
    initializeFactorySettingsFast();
  } else {
    Serial.print("First Byte is set to: ");
    printHex(firstByte);
    Serial.println();
    byte buffer[bufferSize];
    readEEPROMArray(0, buffer, bufferSize);
    Serial.println("Config found:");
    printHexArray(buffer, bufferSize);
  }
}
...
void initializeFactorySettings() {
  ...
  byte buffer[bufferSize];
  readEEPROMArray(0, buffer, bufferSize);
  Serial.println("Config Instantiated.");
  Serial.print(bankCount);
  Serial.println(" copies of:");
  printHexArray(buffer, bufferSize);
}
...
void loadSettingsFromEEPROM() {
  ...
  Serial.print("Active page loaded = ");
  Serial.println(page);
  allLeds(0);
  ledAnimate(page);
  digitalWrite(leds[page], (0x1));
  ...
  Serial.println("USB Channels loaded:");
  printIntArray(usbChannels, channelCount);
  ...
  Serial.println("TRS Channels loaded:");
  printIntArray(trsChannels, channelCount);
  ...
  Serial.println("USB CCs loaded:");
  printIntArray(usbCCs, channelCount);
  ...
  Serial.println("TRS CCs loaded:");
  printIntArray(trsCCs, channelCount);
  ...
  Serial.println("Button USB Channels loaded:");
  printIntArray(buttonUSBChannels, buttonCount);
  ...
  Serial.println("Button TRS Channels loaded:");
  printIntArray(buttonTRSChannels, buttonCount);
  ...
  Serial.println("Button USB Modes loaded:");
  printIntArray(buttonUSBMode, buttonCount);
  ...
  Serial.println("Button TRS Modes loaded:");
  printIntArray(buttonTRSMode, buttonCount);
  ...
  Serial.println("Button USB ParamA loaded:");
  printIntArray(buttonUSBParamA, buttonCount);
  ...
  Serial.println("Button TRS ParamA loaded:");
  printIntArray(buttonTRSParamA, buttonCount);
  ...
  Serial.println("Button USB ParamB loaded:");
  printIntArray(buttonUSBParamB, buttonCount);
  ...
  Serial.println("Button TRS ParamB loaded:");
  printIntArray(buttonTRSParamB, buttonCount);
  ...
  Serial.print ("Setting fadermin to ");
  Serial.println((faderminMSB << 7) + faderminLSB);
  faderMin = (faderminMSB << 7) + faderminLSB;

  int fadermaxLSB = EEPROM.read(6);
  int fadermaxMSB = EEPROM.read(7);

  Serial.print ("Setting fadermax to ");
  Serial.println((fadermaxMSB << 7) + fadermaxLSB);
  faderMax = (fadermaxMSB << 7) + fadermaxLSB;
}
# 1 "/Users/tom/Documents/GitHub/Phone-sized-MidiControl/Firmware/Arduino for MTM 8mu Plus/uMIDI/sysex.ino"
/*
...
void processIncomingSysex(byte* sysexData, unsigned size) {
  Serial.println("Ooh, sysex");
  printHexArray(sysexData, size);
  Serial.println();

  if (size < 3) {
    Serial.println("That's an empty sysex, bored now");
    return;
  }

  // if(!(sysexData[1] == 0x00 && sysexData[2] == 0x58 && sysexData[3] == 0x49)) {
  if (!(sysexData[1] == 0x7d && sysexData[2] == 0x00 && sysexData[3] == 0x00)) {
    Serial.println("That's not a sysex message for us");
    return;
  }
...
void updateAllSettingsAndStoreInEEPROM(byte* newConfig, unsigned size) {
  ...
  Serial.print("Received a new config with size ");
  Serial.println(size);
...
void updateSettingsBlockAndStoreInEEPROM(byte* configFromSysex, unsigned sysexSize, int configStartIndex, int configDataLength, int EEPROMStartIndex) {
  ...
  Serial.print("Storing data of size ");
  Serial.print(configDataLength);
  Serial.print(" at location ");
  Serial.print(EEPROMStartIndex);
  Serial.print(" from data of length ");
  Serial.print(sysexSize);
  Serial.print(" beginning at byte ");
  Serial.println(configStartIndex);
  printHexArray(configFromSysex, sysexSize);
...
void sendCurrentState() {
  ...
  printBytesAsIntsArray(sysexData, howMuchToSend);
  myMidi.sendSysEx(howMuchToSend, sysexData);
  //  myMidi.sendSysEx(bufferSize+8, sysexData);
  forceMidiWrite = true;
}
# 1 "/Users/tom/Documents/GitHub/Phone-sized-MidiControl/Firmware/Arduino for MTM 8mu Plus/uMIDI/utils.ino"
...
int gesture(byte num) {
  switch (num) {
    case 0: // Lift Front
      return ((((IMU_readings[4])<(-16256)?(-16256):((IMU_readings[4])>(0)?(0):(IMU_readings[4]))))>0?(((IMU_readings[4])<(-16256)?(-16256):((IMU_readings[4])>(0)?(0):(IMU_readings[4])))):-(((IMU_readings[4])<(-16256)?(-16256):((IMU_readings[4])>(0)?(0):(IMU_readings[4]))))) >> 2;
      break;

    case 1: // Lift Back
      return ((IMU_readings[4])<(0)?(0):((IMU_readings[4])>(16256)?(16256):(IMU_readings[4]))) >> 2;
      break;

    case 2:// Lift Right
      return ((((IMU_readings[3])<(-16256)?(-16256):((IMU_readings[3])>(0)?(0):(IMU_readings[3]))))>0?(((IMU_readings[3])<(-16256)?(-16256):((IMU_readings[3])>(0)?(0):(IMU_readings[3])))):-(((IMU_readings[3])<(-16256)?(-16256):((IMU_readings[3])>(0)?(0):(IMU_readings[3]))))) >> 2;
      break;

    case 3: // lift Left
      return ((IMU_readings[3])<(0)?(0):((IMU_readings[3])>(16256)?(16256):(IMU_readings[3]))) >> 2;
      break;

    case 4: // Rotate clockwise
      return ((IMU_readings[2] >> 1)<(0)?(0):((IMU_readings[2] >> 1)>(16256)?(16256):(IMU_readings[2] >> 1))) >> 2;
      break;

    case 5: // Rotate Anti-Clockwise
      return ((((IMU_readings[2] >> 1)<(-16256)?(-16256):((IMU_readings[2] >> 1)>(0)?(0):(IMU_readings[2] >> 1))))>0?(((IMU_readings[2] >> 1)<(-16256)?(-16256):((IMU_readings[2] >> 1)>(0)?(0):(IMU_readings[2] >> 1)))):-(((IMU_readings[2] >> 1)<(-16256)?(-16256):((IMU_readings[2] >> 1)>(0)?(0):(IMU_readings[2] >> 1))))) >> 2;
      break;

    case 6: // Turn Over
      return ((IMU_readings[5])<(0)?(0):((IMU_readings[5])>(16256)?(16256):(IMU_readings[5]))) >> 2;
      break;

    case 7: // Right way up
      return ((((IMU_readings[5])<(-16256)?(-16256):((IMU_readings[5])>(0)?(0):(IMU_readings[5]))))>0?(((IMU_readings[5])<(-16256)?(-16256):((IMU_readings[5])>(0)?(0):(IMU_readings[5])))):-(((IMU_readings[5])<(-16256)?(-16256):((IMU_readings[5])>(0)?(0):(IMU_readings[5]))))) >> 2;
      break;
  }
}
...
void calibrateIMU() {
  ...
  Serial.print(BMI160.getXAccelOffset());
  Serial.print("\t");
  Serial.print(BMI160.getYAccelOffset());
  Serial.print("\t");
  Serial.print(BMI160.getZAccelOffset());
  Serial.print("\t");
  Serial.print(BMI160.getXGyroOffset());
  Serial.print("\t");
  Serial.print(BMI160.getYGyroOffset());
  Serial.print("\t");
  Serial.println(BMI160.getZGyroOffset());
  ...
  for (int i = 0; i < 8; i++) {
    for (int j = 0; j < 4; j++) {
      digitalWrite(leds[3 - j], (0x1));
      digitalWrite(leds[4 + j], (0x1));
      delay(30);
      digitalWrite(leds[3 - j], (0x0));
      digitalWrite(leds[4 + j], (0x0));
      delay(30);
    }
  }
}
...
void allLeds(boolean state) {
  for (int ledscan = 0; ledscan < 8; ledscan++) {
...
void ledAnimate(int target) {
  // LED animate
  for (int q = 0; q < 8; q++) {
    digitalWrite(leds[((target + q)<(0)?(0):((target + q)>(8)?(8):(target + q)))], (0x1));
    digitalWrite(leds[((target - q)<(0)?(0):((target - q)>(8)?(8):(target - q)))], (0x1));
    delay(25);
  }
  for (int q = 0; q < 8; q++) {
    digitalWrite(leds[((target + q)<(0)?(0):((target + q)>(8)?(8):(target + q)))], (0x0));
    digitalWrite(leds[((target - q)<(0)?(0):((target - q)>(8)?(8):(target - q)))], (0x0));
    delay(25);
  }
}
...
void checkPage() {
  ...
  if (changingPage) {
    allLeds((0x0));
    digitalWrite(leds[tempPage], (0x1));
  }
